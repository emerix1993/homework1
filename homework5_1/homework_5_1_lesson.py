...
for name, data in car_data.items():
    year, engine_volume, car_type, price = data[1], data[2], data[3], data[4]
    if year >= search_criteria[0] and engine_volume >= search_criteria[1] and price <= search_criteria[2]:
        filtered_cars.append((name, data))
for item in range(len(filtered_cars) - 1):
  for i in range(len(filtered_cars) - item - 1):
    if filtered_cars[i][1][4] > filtered_cars[i + 1][1][4]:
      filtered_cars[i], filtered_cars[i + 1] = filtered_cars[i + 1], filtered_cars[i]
count = 0
